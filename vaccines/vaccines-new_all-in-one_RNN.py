...
df = pd.read_csv(r'.\datasets\vaccinations_italy.csv', index_col = 'date', parse_dates = ['date'], usecols=['daily_vaccinations_raw','date'])
dataset = df.values
#print(dataset)

# using daily_vaccinations_raw as Italy reports data every day, otherwise it is better to use daily_vaccinations that include a 7-day rolling, 
# useful for those countries who don't provide daily data
